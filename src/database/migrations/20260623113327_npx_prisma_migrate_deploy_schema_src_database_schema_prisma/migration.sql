/*
  ...
  - You are about to alter the column `title` on the `notifications` table. The data in that column could be lost. The data in that column will be cast from `VarChar(255)` to `VarChar(191)`.
...
*/
-- AlterTable
ALTER TABLE `notifications` MODIFY `title` VARCHAR(191) NOT NULL,
...
-- RenameIndex
ALTER TABLE `notifications` RENAME INDEX `idx_notifications_package_id` TO `notifications_package_id_idx`;

-- RenameIndex
ALTER TABLE `notifications` RENAME INDEX `idx_notifications_user_id` TO `notifications_user_id_idx`;
